...
Start from the commit to release with a clean worktree, then run:
...
The same script is available as the VS Code task **Create NuGet Release Tag**. `-Force` allows an unclean worktree and skips the confirmation prompt. For non-interactive use, pass `-NewVersion` and `-Force`; use `-WhatIf` to preview the tag operation.
