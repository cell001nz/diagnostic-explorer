...
import { concat, distinctUntilChanged, filter, finalize, map, of, pairwise, repeat, retry, startWith, tap, timer } from 'rxjs';
...
export class DiagnosticsViewComponent implements OnDestroy {
    ...
    private async tryUnsubscribe(process: DiagProcess | undefined) {
        try {
            if (process) await this.#hubService.unsubscribeProcess(process.id);
        } catch (err) {
            console.log(err);
        }
    }
