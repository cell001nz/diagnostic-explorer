...
export class RealtimeModel {
    ...
    constructor() {
        ...
        this.#hubService.processesArrived$.subscribe((p) => {
            ...
            ProcessEventStore.removeMissing(p.map((process) => process.id));
        });
        this.#hubService.processArrived$.subscribe((p: DiagProcess) => {
            ...
            if (found) this.allProcesses.update((procs) => procs.map((x) => (x.id === p.id ? p : x)));
            else this.allProcesses.update((procs) => [...procs, p]);
        });
    }
}
